	...
	uint32_t pixel_fmt, ipu_csi_signal_cfg_t cfg_param)
{
	...
	if (cfg_param.clk_mode == IPU_CSI_CLK_MODE_CCIR656_PROGRESSIVE) {
		...
		#ifdef CONFIG_IWG15
               /* IWG15M-Q7: TV-IN: Adding Both NTSC and PAL Mode */
               if ((width == 720 && height == 625) || (width == 720 && height == 576)) {
                       /* PAL case */
                       /*
                        * Field0BlankEnd = 0x7, Field0BlankStart = 0x3,
                        * Field0ActiveEnd = 0x5, Field0ActiveStart = 0x1
                        */
                       /* IWG15M-Q7: TV-IN: Changing the IPU register values to stop screen polling in PAL mode */
                       ipu_csi_write(ipu, csi, 0xD07DF, CSI_CCIR_CODE_1);
                       /*
                        * Field1BlankEnd = 0x6, Field1BlankStart = 0x2,
                        * Field1ActiveEnd = 0x4, Field1ActiveStart = 0
                        */
                       /* IWG15M-Q7: TV-IN: Changed the IPU register values to stop screen polling in PAL mode */
                       ipu_csi_write(ipu, csi, 0x40596, CSI_CCIR_CODE_2);
                       ipu_csi_write(ipu, csi, 0xFF0000, CSI_CCIR_CODE_3);
                       /* IWG15M-Q7: TV-IN: Adding Both NTSC and PAL Mode */
               } else if ((width == 720 && height == 525) || (width == 720 && height == 480)) {
		#else
