...
void sd_led_ctrl(struct mmc_host *mmchost)
{
	...
	if (boarddata && gpio_is_valid(boarddata->led_gpio)) {
		if ((boarddata->cd_type == ESDHC_CD_PERMANENT) || 
				(gpio_is_valid(boarddata->cd_gpio) &&
				 !gpio_get_value(boarddata->cd_gpio)))
			gpio_set_value (boarddata->led_gpio,1);
		else
			gpio_set_value (boarddata->led_gpio,0);
	}
}
#endif
			 ...
			 struct esdhc_platform_data *boarddata)
{
	...
	#ifdef CONFIG_IWG15
	/* SD LED */
	boarddata->led_gpio = of_get_named_gpio(np, "led-gpios", 0);
	if (gpio_is_valid(boarddata->led_gpio)){
		if (devm_gpio_request_one(&host->mmc->class_dev, 
					boarddata->led_gpio, GPIOF_DIR_OUT,"sd-led"))
			dev_err(mmc_dev(host->mmc),
					"failed to request led-gpios gpio!\n");
	}

	/* SD Power */
	boarddata->pwr_gpio = of_get_named_gpio(np, "pwr-gpios", 0);
	if (gpio_is_valid(boarddata->pwr_gpio)){
		if (devm_gpio_request_one(&host->mmc->class_dev, 
				boarddata->pwr_gpio, GPIOF_DIR_OUT,"sd-pwr"))
			dev_err(mmc_dev(host->mmc),
					"failed to request pwr-gpios gpio!\n");
		/* Set '1' for SDIO & Set '0' for SD/MMC cards */
		gpio_set_value(boarddata->pwr_gpio, 0);
	}
	#endif
