...
static struct mpl3115_data *mpl3115_pdata;
		...
		struct device_attribute *attr, char *buf)
{
	int val;
	u8 sysmode;
#ifdef CONFIG_IWG15_SENSOR_NO_POLL
	struct mpl3115_data *pdata = mpl3115_pdata;
...
static ssize_t mpl3115_read(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct mpl3115_data *pdata = mpl3115_pdata;
	int  pressure = 0;
	short temperature = 0;
	size_t loop = simple_strtoul(buf, NULL, 10);

	mutex_lock(&pdata->data_lock);
		printk("mpl3115:\n");
	/* Check the mpl3115 status */
	if(pdata->active == MPL_STANDBY){
		printk("mpl3115 is inactive\n");
		goto out;
	}

	/* Read the register values of pressure and temparature */
	while (loop--){
		mpl3115_read_data(pdata->client,&pressure,&temperature);
		printk("pressure = 0x%hx, temparature = 0x%hx \n",pressure,temperature);
	};
out:
	mutex_unlock(&pdata->data_lock);
	return count;
}
static DEVICE_ATTR(read, S_IWUSR | S_IRUGO, NULL, mpl3115_read);
		...
		const struct i2c_device_id *id)
{
	...
	input_set_abs_params(idev, ABS_TEMPTERAURE, -0x7FFFFFFF, 0x7FFFFFFF, 0, 0);
        ...
        if (IS_ERR(pdata->hwmon_dev)) {
                ...
                result = PTR_ERR(pdata->hwmon_dev);
		goto err_alloc_data;
        }

        result = sysfs_create_group(&client->dev.kobj, &mpl3115_attr_group);
	if (result) {
...
#ifndef CONFIG_IWG15_SENSOR_NO_POLL
	input_unregister_polled_device(pdata->poll_dev);
#endif
	kfree(pdata);
	return 0;
