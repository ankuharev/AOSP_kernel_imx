...
#ifdef CONFIG_IWG15
/*
 * IWG15M-Q7: LIGHT SENSOR: Include the polling header
 */
...
#ifdef CONFIG_IWG15
/*
 * IWG15M-Q7: LIGHT SENSOR: Polling interval definition
 */
#define POLL_INTERVAL_MIN		1
#define POLL_INTERVAL_MAX		500
#define POLL_INTERVAL			100     /* msecs */

static struct input_polled_dev *isl29023_idev;
...
#ifndef CONFIG_IWG15
/*
 *IWG15M-Q7: LIGHT SENSOR: In polling method interrupt persists, 
 *Interrupt flag and threshold values not required.
 */
...
#ifndef CONFIG_IWG15
/* IWG15M-Q7: LIGHT SENSOR: In polling method threshold settings not required */
/* lux */
...
#ifndef CONFIG_IWG15
/*
 * IWG15M-Q7: LIGHT SENSOR: Comment the interrupt work queue
 */
...
#ifdef CONFIG_IWG15
/*
 * IWG15M-Q7: LIGHT SENSOR: Polling work definition
 */
static void isl29023_dev_poll(struct input_polled_dev *dev)
{
	short out;
	struct i2c_client *client = isl29023_i2c_client;
	out = isl29023_get_adc_value(client);
	input_report_abs(isl29023_idev->input, ABS_MISC, out);
	input_sync(isl29023_idev->input);
}
#endif
				    ...
				    const struct i2c_device_id *id)
{
...
#ifdef CONFIG_IWG15
	/*
	 * IWG15M-Q7: LIGHT SENSOR: Register the device as polling device with 
	 * parameters and comment the device registration as INT device.
	 */
	isl29023_idev = input_allocate_polled_device();
	if (!isl29023_idev) {
		result = -ENOMEM;
		dev_err(&client->dev, "alloc poll device failed!\n");
		goto exit_kfree;
	}
	isl29023_idev->poll = isl29023_dev_poll;
	isl29023_idev->poll_interval = POLL_INTERVAL;
	isl29023_idev->poll_interval_min = POLL_INTERVAL_MIN;
	isl29023_idev->poll_interval_max = POLL_INTERVAL_MAX;
	input_dev = isl29023_idev->input;
	input_dev->name = "isl29023";
	...
	result = input_register_polled_device(isl29023_idev);
	if (result) {
		dev_err(&client->dev, "register poll device failed!\n");
		goto exit_register_polled_device;
	}
	return 0;

exit_register_polled_device:
	input_free_polled_device(isl29023_idev);
#else
...
static int isl29023_remove(struct i2c_client *client)
{
	...
	#ifndef CONFIG_IWG15
	/*
	 * IWG15M-Q7: LIGHT SENSOR: In polling method, functions related to work_sync,
	 * workqueue or irq is not required.
	 */
