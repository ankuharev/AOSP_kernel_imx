...
/*IWG15 : Fix for Image capture issue */ 
/* enable = 0 to turn off AEC/AGC
   enable = 1 to turn on AEC/AGC */
void ov7725_turn_on_AE_AG(int enable)
{
	u8 ae_ag_ctrl;

	udelay(1000);
	ov7725_read_reg(0x10, &ae_ag_ctrl);
	if (enable) {
		/* turn on auto AE/AG */
	...
	ov7725_write_reg(0x10, ae_ag_ctrl);
}
...
static int ioctl_s_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		/*IWG15M-MXM : Fix for Image capture issue  
		 * turn off AE AG for subsampling mode, in case the firmware didn't */
		ov7725_turn_on_AE_AG(0);
